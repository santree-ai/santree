/**
 * The full-width ticket page: the issue a piece of work is about, at reading
 * width — {@link IssueHeader} over the description and comment thread.
 *
 * One page, three hosts: Triage's detail column, the worktree's ticket tab and
 ...
 *
 * A tracker's images need no attachment map: the bodies carry `data:` URIs or
 * public links, unlike a GitHub body's signed uploads (see `MarkdownAttachments`).
 */
...
export function IssuePage({
  ...
  // The tracker's definitive "no such issue" — a branch named like a ticket, a
  // ticket since deleted. Not a failure to report, and the same words the rail's
  // pane uses for it.
  if (data === null) {
    return (
      <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-app">
        <EmptyState title="No linked ticket" subtitle={`${provider} has no issue ${ticketId}.`} />
      </div>
    );
  }
