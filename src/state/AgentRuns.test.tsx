...
/** The project every run in this file happens in. It travels with the call now:
 *  a run outlives the view that started it, so there is nothing to read it back
 *  off later. */
const REPO = "acme/app";
...
describe("AgentRuns", () => {
  it("queues the agent launch directly when setup is off", async () => {
    ...
    await beginRun(() => result.current.beginRun(REPO, "AK-1", "tab-AK-1"));
  ...
  it("prefers a batch's planned setup answer over the preference", async () => {
    ...
    await beginRun(() => result.current.beginRun(REPO, "AK-2", "tab-AK-2"));
    expect(backend.runSetup).not.toHaveBeenCalled();

    // …and a worktree outside the batch still follows the preference.
    await beginRun(() => result.current.beginRun(REPO, "AK-9", "tab-AK-9"));
    expect(backend.runSetup).toHaveBeenCalledTimes(1);
  ...
  it("does not launch an agent when a manual setup re-run finishes", async () => {
    ...
    act(() => result.current.runSetup(REPO, "AK-1"));
