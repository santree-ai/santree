...
santree, bug TODO list, its own Codex session), gated to the developer's GitHub
...
## Codex project config (committed in `.Codex/`)
...
- **Hook:** editing `commands.rs`/`lib.rs`/`domain.rs` triggers a bindings-drift
  reminder (`.Codex/hooks/bindings-reminder.py`).
- Session/worktree state under `.Codex/` stays gitignored; only settings,
...
- **External CLI contracts:** verify vendor flags against `--help`, don't guess.
  Codex `--model` takes dash-form ids or aliases (`sonnet`/`opus`/`haiku`) —
  dotted (`Codex-sonnet-4.5`) is invalid and fails launch.
